...
def kmc2(X, k, chain_length=200, afkmc2=True, random_state=None, weights=None):
    ...
    Args:
      X: (n,d)-shaped numpy.ndarray with data points (or scipy CSR matrix) - assume memmapped
    ...
    cdef numpy.float32_t cand_prob, curr_prob
    cdef numpy.float32_t[::1] q_cand, p_cand, rand_a
    cdef double t1, t2, t
    curr_prob = 0
    cand_prob = 0
    # Handle input
    ...
    cdef const numpy.float32_t[:, :] X_view = X[:]
    sparse = False
    # if weights is None:
    ...
    rel_row = X_view[random_state.choice(X.shape[0], p=weights/weights.sum()), :]
    centers[0, :] = rel_row.todense().flatten() if sparse else rel_row
    if afkmc2:
        di = numpy.min(euclidean_distances(X_view, centers[0:1, :], squared=True), axis=1)*weights
    ...
    for i in range(k-1):
        ...
        dist = euclidean_distances(X_view[cand_ind, :], centers[0:(i+1), :], squared=True)
        # Compute potentials
        ...
        with nogil, cython.boundscheck(False), cython.wraparound(False), cython.cdivision(True):
            ...
            for j in range(q_cand.shape[0]):
                ...
                if j == 0 or curr_prob == 0.0 or cand_prob/curr_prob > rand_a[j]:
                    ...
                    curr_ind = j
                    curr_prob = cand_prob
        rel_row = X_view[cand_ind[curr_ind], :]
...
def do_calc(numpy.int64_t os, int16[:, :] x, int16[:, :] y, int id):
    ...
    cdef int16[:, :] z_view = numpy.empty((x_shape * y_shape, x.shape[1] + y.shape[1] + 3), dtype=numpy.int16)[:]
    # cdef int16 [:, :] z_view = z[:]
    # cdef int16 [:, :] z_view = z_memmap[:]
    cdef int16[:] oh_view = numpy.empty(7, dtype=numpy.int16)[:]
...
cpdef de_dupe_c(int64 strt,int64 x_shape0,int64 y_shape0,int64 ndupes):
    cdef numpy.ndarray[int16, ndim=2] z_view = numpy.empty((x_shape0 * y_shape0, 10), dtype = numpy.int16)
    cdef numpy.ndarray[numpy.float32_t, ndim=2] nd_view = numpy.empty((x_shape0 * (y_shape0-ndupes), 1), dtype = numpy.float32)
    
    
    # cdef int16 [:, :] z_view = z_ram[:]
